# Список изменений `location-dev`:
### Общие изменения
1. Удалены избыточные логи уровня сервисов.
2. Восстановлен маршрут `matrix`.

### Модуль `location`
1. Модуль `location` перенесён в папку `plugins`.
2. Добавлены собственные структуры модуля.
3. Удалён оператор `goto` из `matrix_row_generator`.
4. Добавлены мапперы для: `Wall`, `Sensor`, `Floor`.
5. Частично изменена структура `MatrixInputData`.
6. Исправлен баг назначения усиления антенны по умолчанию, равным 2.
7. Исправлен `math dot import` warning.
8. Исправлен баг генерации чёрной тепловой карты.

### Технические изменения
1. Добавлена возможность получения `get walls detailed` (не для пользователя).
2. Добавлена возможность получения всех `сенсоров` и `стен` (`LIMIT ALL`).
3. Изменён стиль именования интерфейсов для `сервисов` и `юзкейсов`.
4. Добавлена возможность обновления тепловой карты (не для пользователя).
5. Исправлен баг, вызывающий ошибку при попытке удаления несуществующей тепловой карты.

## Обнаруженные ошибки
1. Конфликт единиц измерения сенсоров, стен и пр.
2. Генерируется некорректная тепловая карта.


# Список изменений `experiment`:

1. Перенесена точка входа в приложение и изменён Dockerfile.
...
27. Восстановлены маршруты для `Sensor`: создание, получение по ID, получение списка, получение детализированного объекта по ID, получения списка детализированных объектов,  мягкое удаление, восстановление, patch update.
28. Добавлен **технический** маршрут для `Sensor`: получение по MAC.
29. Исправлен баг, при котором в случае отсутствия радио интерфейсов у ТД и сенсоров возвращалась ошибка сервера со статусом 500. Теперь поле **radios** в таких ситуациях содержит **null**.
30. Добавлены маршруты для `SensorRadioTemplate`: создание, получение по ID, получение списка,  мягкое удаление, восстановление, patch update. Группа: `/sensor-radio-template`.
31. Добавлены маршруты для `SensorRadio`: создание, получение по ID, получение списка,  мягкое удаление, восстановление, patch update. Группа: `/sensor-radio`.
32. Исправлен баг с возвращением статуса 200 при обновлении несуществующей сущности. Теперь в такой ситуации возвращается 404.
33. Исправлен баг с patch update.
34. Исправлен баг с отсутствием проверки на уникальность MAC при создании сенсора. 
35. Добавлены новые баги :3
