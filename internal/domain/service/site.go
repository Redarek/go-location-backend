...
func (s *siteService) GetSiteDetailed(ctx context.Context, getDTO *dto.GetSiteDetailedDTO) (siteDetailed *entity.SiteDetailed, err error) {
	...
	buildings, err := s.buildingRepo.GetAll(ctx, getDTO.ID, getDTO.Size, utils.GetOffset(getDTO.Page, getDTO.Size))
	if err != nil {
		if !errors.Is(err, ErrNotFound) {
			return
		}
	...
	wallTypes, err := s.wallTypeRepo.GetAll(ctx, getDTO.ID, getDTO.Size, utils.GetOffset(getDTO.Page, getDTO.Size))
	if err != nil {
		if !errors.Is(err, ErrNotFound) {
			return
		}
	...
	accessPointTypes, err := s.accessPointTypeRepo.GetAll(ctx, getDTO.ID, getDTO.Size, utils.GetOffset(getDTO.Page, getDTO.Size))
	if err != nil {
		if !errors.Is(err, ErrNotFound) {
			return
		}
	...
	sensorTypes, err := s.sensorTypeRepo.GetAll(ctx, getDTO.ID, getDTO.Size, utils.GetOffset(getDTO.Page, getDTO.Size))
	if err != nil {
		if !errors.Is(err, ErrNotFound) {
			return
		}
...
func (s *siteService) GetSites(ctx context.Context, dto *dto.GetSitesDTO) (sites []*entity.Site, err error) {
	sites, err = s.siteRepo.GetAll(ctx, dto.UserID, dto.Size, utils.GetOffset(dto.Page, dto.Size))
	if err != nil {
		if errors.Is(err, ErrNotFound) {
...
func (s *siteService) GetSitesDetailed(ctx context.Context, getDTO *dto.GetSitesDTO) (sitesDetailed []*entity.SiteDetailed, err error) {
	...
	for _, site := range sites {
		buildings, err := s.buildingRepo.GetAll(ctx, site.ID, getDTO.Size, utils.GetOffset(getDTO.Page, getDTO.Size))
		if err != nil {
			if !errors.Is(err, ErrNotFound) {
				return nil, err
			}
		...
		wallTypes, err := s.wallTypeRepo.GetAll(ctx, site.ID, getDTO.Size, utils.GetOffset(getDTO.Page, getDTO.Size))
		if err != nil {
			if !errors.Is(err, ErrNotFound) {
				return nil, err
			}
		...
		accessPointTypes, err := s.accessPointTypeRepo.GetAll(ctx, site.ID, getDTO.Size, utils.GetOffset(getDTO.Page, getDTO.Size))
		if err != nil {
			if !errors.Is(err, ErrNotFound) {
				return nil, err
			}
		...
		sensorTypes, err := s.sensorTypeRepo.GetAll(ctx, site.ID, getDTO.Size, utils.GetOffset(getDTO.Page, getDTO.Size))
		if err != nil {
			if !errors.Is(err, ErrNotFound) {
				return nil, err
			}
