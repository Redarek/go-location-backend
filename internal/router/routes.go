...
func RegisterRoutes(router *Router, handlerComposite *composites.HandlerComposite) {
	...
	floor := v1.Group("/floor", middleware.Auth)
	handlerComposite.FloorHandler.Register(&floor)

	wallType := v1.Group("/wallType", middleware.Auth)
	handlerComposite.WallTypeHandler.Register(&wallType)
	...
	accessPointRadio := v1.Group("/ap-radio", middleware.Auth)
	handlerComposite.AccessPointRadioHandler.Register(&accessPointRadio)

	sensorType := v1.Group("/sensorType", middleware.Auth)
	handlerComposite.SensorTypeHandler.Register(&sensorType)

	sensorRadioTemplate := v1.Group("/sensor-radio-template", middleware.Auth)
	handlerComposite.SensorRadioTemplateHandler.Register(&sensorRadioTemplate)
